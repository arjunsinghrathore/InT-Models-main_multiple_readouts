...
class FFhGRU(nn.Module):
    ...
    def __init__(self, dimensions, timesteps=8, kernel_size=15, jacobian_penalty=False, grad_method='bptt', no_inh=False, lesion_alpha=False, lesion_mu=False, lesion_gamma=False, lesion_kappa=False, nl=F.softplus, classes = 3):
        ...
        self.readout_conv = nn.Conv2d(dimensions, 3, 1)
        self.target_conv = nn.Conv2d(4, 3, 5, padding=5 // 2)
        ...
        self.readout_dense = nn.Linear(3, 3)
...
class FFhGRU_multi(nn.Module):

    def __init__(self, dimensions, timesteps=8, kernel_size=15, jacobian_penalty=False, grad_method='bptt', no_inh=False, lesion_alpha=False, lesion_mu=False, lesion_gamma=False, lesion_kappa=False, nl=F.softplus, classes = 3):
        '''
        '''
        super(FFhGRU_multi, self).__init__()
        self.timesteps = timesteps
        self.jacobian_penalty = jacobian_penalty
        self.grad_method = grad_method
        self.hgru_size = dimensions
        ...
        # self.readout_conv_1 = nn.Conv2d(dimensions, 1, 1)
        # self.target_conv_1 = nn.Conv2d(2, 1, 5, padding=5 // 2)
        # torch.nn.init.zeros_(self.target_conv_1.bias)
        # # self.target_conv_0 = nn.Conv2d(3, 16, 5, padding=0)  # padding=5 // 2)
        # # self.target_pool_0 = nn.MaxPool2d(2, 2, padding=0)
        # # self.target_conv_1 = nn.Conv2d(16, 16, 5, padding=0)  # padding=7 // 2)
        # # self.target_pool_1 = nn.MaxPool2d(2, 2, padding=0)
        ...
        # # self.readout_dense = nn.Linear(1, 1)
        # # Multi-Class Classufication
        # self.readout_dense_1 = nn.Linear(1, 1)

        # ############## 2nd READOUT ###########################
        
        # self.readout_conv_2 = nn.Conv2d(dimensions, 1, 1)
        # self.target_conv_2 = nn.Conv2d(2, 1, 5, padding=5 // 2)
        # torch.nn.init.zeros_(self.target_conv_2.bias)
        # # self.target_conv_0 = nn.Conv2d(3, 16, 5, padding=0)  # padding=5 // 2)
        # # self.target_pool_0 = nn.MaxPool2d(2, 2, padding=0)
        # # self.target_conv_1 = nn.Conv2d(16, 16, 5, padding=0)  # padding=7 // 2)
        # # self.target_pool_1 = nn.MaxPool2d(2, 2, padding=0)
        ...
        # # self.readout_dense = nn.Linear(1, 1)
        # # Multi-Class Classufication
        # self.readout_dense_2 = nn.Linear(1, 1)
        # # torch.nn.init.zeros_(self.readout_dense.bias)
        # self.nl = nl

        # ############## List of READOUTS ###########################

        self.readout_conv_list = nn.ModuleList([nn.Conv2d(dimensions, 1, 1) for _ in range(self.classes-1)])
        self.target_conv_list = nn.ModuleList([nn.Conv2d(2, 1, 5, padding=5 // 2) for _ in range(self.classes-1)])
        for i in range(self.classes-1):
            torch.nn.init.zeros_(self.target_conv_list[i].bias)

        self.readout_dense_list = nn.ModuleList([nn.Linear(1, 1) for _ in range(self.classes-1)])
        self.nl = nl
    ...
    def forward(self, x, testmode=False):
        ...
        states_list = [[] for _ in range(self.classes-1)]
        ...
        for t in range(x_shape[2]):
            ...
            if testmode:
                inhibition, excitation, gate = out
                gates.append(gate)  # This should learn to keep the winner
                # states_1.append(self.readout_conv_1(excitation))  # This should learn to keep the winner
                # states_2.append(self.readout_conv_2(excitation))  # This should learn to keep the winner
                for i in range(self.classes):
                    states_list[i].append(self.readout_conv_list[i](excitation))
            else:
                inhibition, excitation = out
                # states.append(self.readout_conv(excitation))
                # if t == x_shape[2] - 1:
        ...
        # output_1 = torch.cat([self.readout_conv_1(excitation), x[:, 2, 0][:, None]], 1)
        # # output = torch.cat(states, 1).mean(1, keepdim=True)  # Convert video -> spatial path
        # # output = torch.cat([output, x[:, 1:, 0]], 1)  # Paste the start and end goals

        # # Potentially combine target_conv + readout_bn into 1
        # output_1 = self.target_conv_1(output_1)  # output.sum(1, keepdim=True))  # 2 channels -> 1. Is the dot in the target?
        # # output = self.target_conv(torch.cat([excitation, x[:, 2, 0][:, None]], 1))
        # # # Mini readout network for solving PF without any clutter. RF=32x32
        # # output = self.nl(self.target_conv_0(output))
        # # output = self.target_pool_0(output)
        ...
        # # if sum([*output.size()[2:]]) > 2:
        # #     output = F.avg_pool2d(output, kernel_size=output.size()[2:])  # Spatial pool
        # output_1 = F.avg_pool2d(output_1, kernel_size=output_1.size()[2:])
        # # output = self.readout_dense(output.reshape(x_shape[0], -1))  # scale + intercept
        # output_1 = output_1.reshape(x_shape[0], -1)
        # output_1 = self.readout_dense_1(output_1)

        # ############## 2nd READOUT ###########################

        # output_2 = torch.cat([self.readout_conv_2(excitation), x[:, 2, 0][:, None]], 1)
        # # output = torch.cat(states, 1).mean(1, keepdim=True)  # Convert video -> spatial path
        # # output = torch.cat([output, x[:, 1:, 0]], 1)  # Paste the start and end goals

        # # Potentially combine target_conv + readout_bn into 1
        # output_2 = self.target_conv_2(output_2)  # output.sum(1, keepdim=True))  # 2 channels -> 1. Is the dot in the target?
        # # output = self.target_conv(torch.cat([excitation, x[:, 2, 0][:, None]], 1))
        # # # Mini readout network for solving PF without any clutter. RF=32x32
        # # output = self.nl(self.target_conv_0(output))
        # # output = self.target_pool_0(output)
        ...
        # # if sum([*output.size()[2:]]) > 2:
        # #     output = F.avg_pool2d(output, kernel_size=output.size()[2:])  # Spatial pool
        # output_2 = F.avg_pool2d(output_2, kernel_size=output_2.size()[2:])
        # # output = self.readout_dense(output.reshape(x_shape[0], -1))  # scale + intercept
        # output_2 = output_2.reshape(x_shape[0], -1)
        # output_2 = self.readout_dense_2(output_2)
        ...
        for i in range(self.classes-1):
            temp_output = torch.cat([self.readout_conv_list[i](excitation), x[:, 2, 0][:, None]], 1)

            # Potentially combine target_conv + readout_bn into 1
            temp_output = self.target_conv_list[i](temp_output)  # output.sum(1, keepdim=True))  # 2 channels -> 1. Is the dot in the target?

            temp_output = F.avg_pool2d(temp_output, kernel_size=temp_output.size()[2:])

            temp_output = temp_output.reshape(x_shape[0], -1)
            temp_output = self.readout_dense_list[i](temp_output)
        ...
        if testmode: return output_1, output_2, torch.stack(states_1, 1), torch.stack(states_2, 1), torch.stack(gates, 1)

        # return output_1, output_2, jv_penalty
        return outputs, jv_penalty
